...
//MUI
import { Grid, Modal, InputLabel, Select, MenuItem, FormControl, TableContainer,Table, Paper, TableHead, TableBody, TableRow, TableCell, Button} from "@mui/material"
...
export default function ModalTests() {
	...
	return <>
		<Grid container  alignItems="center" justifyContent="center" className='basicBorders'>
			<Grid item xs={2}><Button variant='contained' onClick={handleOpen} className='centrate'>Añadir Examen de Sangre</Button></Grid>
			<Grid item xs={10}>{activeHematologyState.map((e) => <h2 className='basicBorders' key={e.body}>{e.body}</h2>)}</Grid>
		</Grid>
			


			<Modal open={open} disableScrollLock={false} onClose={handleClose} >
